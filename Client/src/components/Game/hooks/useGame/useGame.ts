...
export const useGame = () => {
  ...
  const move = (
    ...
    setTimeout(endOfMoveAction, animationDuration);
  };
  ...
  // Add the setGameState method to the hook.
  const setGameState = useCallback((newState: State) => {
    dispatch({ type: "SET_STATE", newState });
  }, []);

  return [tileList, moveLeft, moveRight, moveUp, moveDown, getGameState, setGameState] as [
    TileMeta[],
    () => void,
    () => void,
    () => void,
